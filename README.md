# p5_tutorials

code I wrote while I was at [Rune Madsen's](https://runemadsen.com/) [Programming Design Systems workshop](https://github.com/runemadsen/programming-design-systems-workshop), reading [his book on the same topic](https://programmingdesignsystems.com/), watching [The Coding Train's videos about p5.js](https://www.youtube.com/user/shiffman/playlists?sort=dd&view=50&shelf_id=14) (tutorial code [here](https://github.com/CodingTrain/Rainbow-Code/tree/master/Tutorials/P5JS)), and doing other things.

## Contents

### Larger Pieces

- [rainbow-o-matic](rainbow-o-matic)
- [mta_subway_brochure](mta_subway_brochure)
- [tape+cardboard](tape+cardboard)
- [walks](walks)

### Templates

- [empty-example](empty-example)
- [non-empty-example](non-empty-example)

### From/For Rune Madsen's Workshop/Book

- [rotation](rotation)
- [slim](slim)
- [basic_shapes](basic_shapes)
- [custom_shapes](custom_shapes)
- [icecream_cone](icecream_cone)
- [polygon_slider_random](polygon_slider_random)
- [color_slider](color_slider)
- [color_slider_hsluv](color_slider_hsluv)
- [color_slider_hsluv_comparison](color_slider_hsluv_comparison)

### Coding Train

- [hello_world](hello_world)
- [dada](dada)
- [camera](camera)
- [lightbulb](lightbulb)
- [vase](vase)

## You'll want

- [p5.js](https://p5js.org/download/)
- [hsluv.js](https://github.com/hsluv/hsluv/releases) for perceptually uniform color spaces (read more about that [here](https://programmingdesignsystems.com/color/perceptually-uniform-color-spaces/))
